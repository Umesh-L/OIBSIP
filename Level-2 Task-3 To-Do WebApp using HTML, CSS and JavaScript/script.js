...
class UIManager {
  ...
  displayTodos(todos) {
    ...
    if (todos.length === 0) {
      this.todosListBody.innerHTML = `
        <tr><td colspan="5" class="text-center">No task found</td></tr>`;
      return;
    }
  ...
  showAlertMessage(message, type) {
    const alertBox = `
      <div class="alert alert-${type} shadow-lg mb-5 w-full show">
        <div><span>${message}</span></div>
      </div>`;
    this.alertMessage.innerHTML = alertBox;

    const alertElement = this.alertMessage.querySelector(".alert");
